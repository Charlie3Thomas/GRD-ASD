...
namespace UnityEditor.Localization.Samples
{
    ...
    {
        [MenuItem("Localization Samples/Locales/Create French Locale")]
        static void CreateFrench()
        {
            // Create a locale to represent Japanese.
            var locale = Locale.CreateLocale(SystemLanguage.French);

            // Customize the name.
            locale.name = "French";

            var path = EditorUtility.SaveFilePanelInProject("Save French Locale Asset", locale.name, "asset", null);
